 ...
 * Integration: Stripe Checkout Test Button + Discount Code + Massive Headers
 */
...
const TestCheckoutButton = () => {
  ...
  const handleBuy = async () => {
    ...
    try {
      const res = await fetch("/api/create-checkout-session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ priceId: "price_1SwvOSKFN6WMOhlF5xerUfID" })
      });
      const data = await res.json();
      if (data.url) {
        window.location.href = data.url;
      } else {
        setError(data.error || "Internal System Leak");
      }
    } catch (e) {
  ...
  return (
    <div className="w-full py-16 px-6 border-t border-[#c58158]/10 bg-[#0a0604] flex flex-col items-center space-y-6">
      <div className="flex items-center gap-2 text-[#c58158]/30">
        <AlertCircle size={14} />
        <p className="text-[10px] font-black uppercase tracking-[0.4em] italic text-center">INTERNAL SHOP TEST SECTION - TEMPORARY</p>
      </div>
      <button 
        onClick={handleBuy}
        disabled={loading}
        className="bg-white/5 border border-white/10 text-white/50 px-8 py-3 rounded text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all flex items-center gap-3 disabled:opacity-50"
      >
        {loading ? <Loader2 size={14} className="animate-spin" /> : <Package size={14} />}
        Buy PRESSURE (STRIPE TEST)
      </button>
      {error && <p className="text-red-900 font-black text-[10px] uppercase italic tracking-widest">{error}</p>}
    </div>
  );
};
    ...
    <div className="bg-[#1a0f0a] text-[#f4e4bc] font-serif relative overflow-x-hidden min-h-screen">
      ...
      <section className="relative min-h-[85vh] flex items-center justify-center px-8 pt-24 pb-12 overflow-hidden text-center">
        ...
        <div className="max-w-6xl mx-auto relative z-20 w-full grid lg:grid-cols-2 gap-12 items-center text-center">
          <div className="text-center lg:text-left">
            <ScrollReveal>
              <div className="flex flex-col items-center lg:items-start">
                <div className="mb-6 sm:mb-10"><img src="https://images.travelprox.com/mrplumberman/plumlogo.png" className="h-16 sm:h-20 md:h-28 w-auto object-contain" alt="Mr. Plumber Man Nutrition" /></div>
                ...
                {/* HERO HEADLINE - THE ORIGINAL BIG SIZE */}
                <h1 className="text-6xl sm:text-7xl md:text-9xl font-black tracking-tighter mb-10 leading-[0.9] uppercase italic text-white drop-shadow-2xl text-center lg:text-left w-full break-words">
                  MASTER YOUR FLOW <br className="hidden md:block" /> <span className="text-[#d4af37]">LIKE A PRO.</span>
                </h1>
                
                <div className="space-y-6 max-w-2xl text-center lg:text-left">
                  <p className="text-xl sm:text-2xl md:text-3xl text-[#f4e4bc]/80 leading-relaxed font-bold italic">Performance engineered for men who like everything running smoothly.</p>
          ...
          <div className="relative group lg:mt-0 mt-16">
            <ScrollReveal>
      ...
      <div className="w-full bg-[#140b08] border-y border-[#c58158]/30 py-6 relative overflow-hidden text-center z-30 px-8">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-center gap-6 md:gap-16 relative z-10">
          <div className="flex items-center gap-4 text-center"><MapPin size={18} className="text-[#c58158]" /><p className="text-[#f4e4bc] text-xs sm:text-sm font-black uppercase tracking-[0.2em] italic">Recently purchased by customers across the U.S.</p></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-10 text-center">
              {[
                ...
                <div key={i} className="bg-[#2a1b15]/40 border-2 border-[#c58158]/20 p-6 sm:p-10 flex flex-col hover:border-[#d4af37] transition duration-500 rounded-sm shadow-2xl group text-left">
                  <div className="flex items-center gap-4 sm:gap-5 mb-8 text-left"><div className="w-10 h-10 sm:w-14 sm:h-14 bg-[#1a0f0a] border border-[#c58158]/40 rounded-lg flex items-center justify-center text-[#d4af37] group-hover:scale-110 transition-transform shrink-0"> {d.icon}</div><h3 className="text-xl sm:text-2xl font-black uppercase italic tracking-tighter text-white leading-tight">{d.title}</h3></div>
                  <div className="space-y-6">
                    <div className="pt-8 border-t border-[#c58158]/10 text-left"><p className="text-[10px] uppercase tracking-widest text-[#d4af37] font-black mb-3">Recommended Part:</p><p className="text-2xl sm:text-3xl font-black text-white italic uppercase tracking-tighter mb-4 leading-none">{d.fix}</p><p className="text-[10px] sm:text-xs font-bold text-[#f4e4bc]/50 italic leading-relaxed uppercase">{d.action}</p></div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="bg-[#1a0f0a] py-16 sm:py-32 relative overflow-hidden text-center px-10">
        <ScrollReveal>
          <div className="max-w-4xl mx-auto relative z-10 text-center">
             ...
             <p className="text-lg sm:text-3xl text-[#f4e4bc]/60 font-bold max-w-3xl mx-auto leading-relaxed italic uppercase tracking-widest mb-12 sm:mb-16 text-center text-center">If you change your mind on a kit, we'll take it back. We offer a full refund on any unopened and unused product within 30 days of purchase. No clogs in the process.</p>
          ...
          <div className="max-w-6xl mx-auto text-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 items-stretch text-right">
              <div className="bg-[#2a1b15]/40 p-8 sm:p-16 border-2 border-[#c58158]/10 relative z-10 shadow-lg group grayscale opacity-85 hover:opacity-100 transition-all duration-500 text-right">
                <div className="absolute top-6 left-6 text-[9px] sm:text-[10px] uppercase tracking-widest text-[#c58158]/60 text-left">Mass Market Formula</div>
                <h4 className="text-lg sm:text-2xl font-black text-[#c58158] mb-10 sm:mb-12 flex items-center uppercase tracking-[0.2em] italic justify-end"><XCircle className="w-6 h-6 mr-3 text-red-900" /> NUGENIX TOTAL-T</h4>
                <div className="space-y-6 sm:space-y-8 italic text-right">
                  {[{ l: "Price Comparison", v: "$79+ (Retail Bloat)" }, { l: "Magnesium Form", v: "Oxide (Gastro Filler)" }, { l: "Zinc Form", v: "Oxide (Low Absorption)" }, { l: "Tongkat Ali", v: "50mg Raw Root Powder" }, { l: "Saw Palmetto", v: "50mg Raw Berries" }, { l: "Extract Type", v: "Non-Standardized" }].map((row, i) => (
                    <div key={i} className="flex justify-between border-b border-[#c58158]/10 pb-4 sm:pb-5 text-right"><p className="text-[10px] sm:text-xs text-[#c58158]/60 uppercase font-black tracking-widest text-left">{row.l}</p><p className="text-xs sm:text-base text-[#f4e4bc]/40 font-bold uppercase text-right">{row.v}</p></div>
                  ))}
                </div>
              </div>
              <div className="relative bg-[#1a0f0a] p-8 sm:p-16 border-[3px] border-[#c58158] shadow-[0_0_100px_rgba(197,129,88,0.15)] z-20 overflow-hidden text-right">
                <h4 className="text-lg sm:text-2xl font-black text-white mb-10 sm:mb-12 flex items-center relative z-10 uppercase tracking-[0.2em] italic justify-end text-right"><ShieldCheck className="w-6 h-6 mr-3 text-[#d4af37]" /> MR. PLUMBER MAN</h4>
                <div className="space-y-6 sm:space-y-8 relative z-10 italic">
                  {[{ l: "Price Comparison", v: "$59 (Direct Value)" }, { l: "Magnesium Form", v: "Glycinate (High Torque)" }, { l: "Zinc Form", v: "Picolinate (Bioavailable)" }, { l: "Tongkat Ali", v: "200mg Standardized" }, { l: "Saw Palmetto", v: "100mg Standardized" }, { l: "Extract Type", v: "Potency Guaranteed" }].map((row, i) => (
                    <div key={i} className="flex justify-between border-b border-[#c58158]/20 pb-4 sm:pb-5 text-right"><p className="text-[10px] sm:text-xs text-[#d4af37] uppercase font-black tracking-widest text-left">{row.l}</p><p className="text-xs sm:text-base text-white font-black uppercase text-right">{row.v}</p></div>
                  ))}
                </div>
              </div>
            </div>
      ...
      {/* Upgrade CTA */}
      <section className="bg-[#1a0f0a] py-20 sm:py-32 px-8 border-b border-[#c58158]/10 text-center">
        <ScrollReveal>
          <div className="max-w-4xl mx-auto space-y-10 sm:space-y-12">
            <h2 className="text-3xl sm:text-6xl md:text-8xl font-black uppercase text-white tracking-tighter leading-none italic">STOP BUYING FILLER. <br /><span className="text-[#c58158]">UPGRADE YOUR SYSTEM.</span></h2>
            <button onClick={() => scrollTo(depotRef)} className="bg-[#c58158] text-[#1a0f0a] px-10 sm:px-12 py-5 sm:py-6 font-black uppercase tracking-widest text-sm sm:text-base shadow-[0_10px_0_#8c5a3d] rounded-lg hover:translate-y-[2px] active:translate-y-[8px] transition-all mx-auto flex justify-center items-center italic text-center">Shop The Supply Depot</button>
          </div>
        </ScrollReveal>
      </section>
      ...
      <section ref={depotRef} className="px-6 py-16 sm:py-32 bg-[#140b08] text-center">
       <div className="max-w-7xl mx-auto italic text-center">
          <ScrollReveal>
            <div className="mb-16 sm:mb-24 space-y-6 text-center">
               <h2 className="text-[#c58158] font-black uppercase tracking-[0.6em] text-xs underline decoration-1 text-center">Supply Inventory</h2>
               ...
               <div className="flex items-center gap-3 justify-center text-[#d4af37] animate-pulse"><Truck size={20} sm:size={24} /><span className="text-xs sm:text-sm font-black uppercase tracking-[0.5em] text-center">All orders include free express shipping</span></div>
            </div>
            <div className="grid md:grid-cols-3 gap-8 sm:gap-10 text-center">
               {[
                 ...
                 <div key={p.id} className="bg-[#2a1b15]/40 border-2 border-[#c58158]/20 p-8 sm:p-10 hover:border-[#d4af37] transition duration-500 flex flex-col items-center group rounded-sm shadow-2xl relative overflow-hidden text-center">
                    {p.tag && (<div className="absolute top-4 left-[-30px] bg-[#d4af37] text-[#1a0f0a] px-10 py-1 text-[8px] font-black uppercase tracking-widest -rotate-45 shadow-lg">{p.tag}</div>)}
                    <div className="w-full aspect-square bg-[#1a0f0a] border border-[#c58158]/20 mb-6 sm:mb-8 flex items-center justify-center relative shadow-inner overflow-hidden"><img src={p.imgOverride} alt={p.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" /></div>
                    ...
                    <div className="mt-auto w-full space-y-4 sm:space-y-6 pt-6 sm:pt-8 border-t border-[#c58158]/20 text-center">
                       <div className="flex flex-col items-center gap-2 text-center">
                          <p className="text-3xl sm:text-4xl font-black italic text-white">${p.price}</p>
                          <span className="text-[10px] text-[#c58158] font-black uppercase tracking-widest">Free Express Shipping</span>
                       </div>
                       <div className="space-y-3 sm:space-y-4 text-center">
                         <button className="w-full bg-[#c58158] text-[#1a0f0a] py-3 sm:py-4 font-black uppercase tracking-widest text-xs hover:bg-[#d4af37] shadow-[0_8px_0_#8c5a3d] active:translate-y-[8px] transition-all text-center">Add To Kit</button>
                         <button className="w-full border border-[#c58158]/40 bg-transparent text-[#d4af37] py-2 font-black uppercase tracking-[0.3em] text-[9px] hover:bg-[#c58158]/10 transition-all italic flex items-center justify-center gap-2 text-center"><RefreshCcw size={12} sm:size={14} /> Subscribe & Save 10%</button>
                       </div>
                    </div>
                 </div>
               ))}
      ...
      <footer className="bg-[#0f0a08] py-16 sm:py-24 px-10 text-center italic border-t border-[#c58158]/10 text-center">
         <div className="max-w-5xl mx-auto space-y-10 sm:space-y-12 flex flex-col items-center text-center">
            <div className="opacity-50 grayscale hover:opacity-100 transition-opacity mb-4 sm:mb-6"><img src="https://images.travelprox.com/mrplumberman/plumlogo.png" className="h-10 sm:h-12 w-auto" alt="Mr. Plumber Man Logo" /></div>
            <p className="text-[10px] sm:text-xs font-black uppercase tracking-[0.2em] text-[#c58158]/30 leading-relaxed max-w-2xl mx-auto italic">
      ...
      {/* TEST SECTION (Temporary Internal Mechanic) */}
      <TestCheckoutButton />
