 ...
 * Fix: Mismatched JSX tags and structural integrity.
 */
  ...
  return (
    <div className="space-y-4">
      {faqData.map((item, idx) => (
        <div key={idx} className={`border-2 transition-all duration-300 rounded-sm overflow-hidden ${openIndex === idx ? "bg-[#2a1b15] border-[#d4af37]" : "bg-[#2a1b15]/40 border-[#c58158]/20"}`}>
          <button onClick={() => setOpenIndex(openIndex === idx ? null : idx)} className="w-full flex items-center justify-between p-6 text-left">
            <span className={`text-sm lg:text-base font-black uppercase tracking-widest italic ${openIndex === idx ? "text-[#d4af37]" : "text-white"}`}>{item.q}</span>
            <ChevronDown size={20} className={`transition-transform ${openIndex === idx ? "rotate-180 text-[#d4af37]" : "text-[#c58158]"}`} />
          </button>
          <div className={`transition-all duration-300 overflow-hidden ${openIndex === idx ? "max-h-[300px] opacity-100" : "max-h-0 opacity-0"}`}>
            <div className="px-6 pb-6 pt-2 border-t border-[#c58158]/10">
              <p className="text-[#f4e4bc]/80 text-sm lg:text-base font-bold italic leading-relaxed">{item.a}</p>
            </div>
          </div>
        </div>
      ))}
...
const DiscountPopup = ({ isOpen, onClose, onUnlock }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center px-4 bg-[#1a0f0a]/80 backdrop-blur-md animate-in fade-in duration-500">
      <div className="relative w-full max-w-[420px] bg-[#2a1b15] border-2 border-[#c58158] rounded-[40px] shadow-2xl p-8 sm:p-12 text-center">
        <button onClick={onClose} className="absolute top-6 right-6 text-[#c58158] hover:text-[#d4af37]"><X size={24} /></button>
        {isSubmitted ? (
          <div className="flex flex-col items-center space-y-6">
            <div className="w-16 h-16 bg-[#d4af37]/20 rounded-full flex items-center justify-center text-[#d4af37]"><CheckCircle2 size={32} /></div>
            <h2 className="text-3xl font-black italic uppercase text-white">SYSTEM CLEAR</h2>
            <div className="bg-[#1a0f0a] border-2 border-dashed border-[#c58158] p-6 rounded-2xl w-full">
              <p className="text-4xl font-black text-white italic tracking-widest">Plumber20</p>
            </div>
            <button onClick={onClose} className="relative group w-full bg-gradient-to-b from-[#d4af37] to-[#c58158] py-4 rounded-full font-black uppercase text-xs italic"><span className="relative z-10">Back to Depot</span><div className="btn-glare-overlay" /></button>
          </div>
        ) : (
          <div className="flex flex-col items-center">
            <img src="https://images.travelprox.com/mrplumberman/plumlogo.png" className="h-10 w-auto mb-8 grayscale" alt="Logo" />
            ...
            <form onSubmit={handleSubmit} className="w-full space-y-4">
              <input className="w-full bg-white px-6 py-4 rounded-full text-sm font-bold placeholder-[#1a0f0a]/40" placeholder="First Name" type="text" required />
              <input className="w-full bg-white px-6 py-4 rounded-full text-sm font-bold placeholder-[#1a0f0a]/40" placeholder="Email Address" type="email" required />
              <button type="submit" className="group relative w-full bg-black text-white border border-[#c58158]/40 py-5 rounded-full font-black uppercase text-sm italic"><span className="relative z-10">SIGN UP FOR 20% OFF</span><div className="btn-glare-overlay" /></button>
            </form>
          </div>
        )}
      </div>
...
const DiscountBadge = () => {
  ...
  return (
    <button onClick={handleCopy} className="fixed bottom-6 right-6 z-[150] bg-[#1a0f0a] border-2 border-[#d4af37] rounded-full px-5 py-3 flex items-center gap-3 shadow-[0_0_20px_rgba(212,175,55,0.2)] transition-all">
    ...
    <>
      <nav className="fixed top-0 left-0 w-full z-[80] p-4 sm:p-6">
        <div className="max-w-7xl mx-auto flex justify-between items-center bg-[#2a1b15]/95 backdrop-blur-md border border-[#c58158]/30 rounded-full px-4 sm:px-6 py-2 sm:py-3 shadow-2xl">
          ...
          <div className="flex gap-4 items-center">
            <button onClick={() => scrollTo(benefitsRef)} className="hidden md:block text-[10px] font-black uppercase tracking-[0.3em] text-[#f4e4bc] hover:text-[#d4af37]">Specs</button>
            <button onClick={() => scrollTo(depotRef)} className="relative overflow-hidden bg-gradient-to-b from-[#d4af37] to-[#c58158] text-[#1a0f0a] px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest italic group"><span className="relative z-10">Supply Depot</span><div className="btn-glare-overlay" /></button>
          </div>
        </div>
      </nav>
      ...
      <section className="relative min-h-[95vh] lg:min-h-screen flex items-center justify-center px-6 pt-20 lg:pt-36 pb-6 lg:pb-12 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-20 w-full grid lg:grid-cols-2 gap-0 lg:gap-24 items-center">
          <div className="text-center lg:text-left flex flex-col items-center lg:items-start order-1">
            <ScrollReveal>
              <img src="https://images.travelprox.com/mrplumberman/plumlogo.png" className="h-10 lg:h-32 w-auto mb-10 hidden lg:block" alt="Logo" />
              ...
              <h1 className="text-[10.5vw] lg:text-7xl xl:text-8xl 2xl:text-9xl font-black tracking-tight mb-1 lg:mb-6 leading-[0.85] uppercase italic text-white drop-shadow-2xl">
                <span className="whitespace-nowrap">READY WHEN</span> <br /> <span className="text-[#d4af37] whitespace-nowrap">IT COUNTS.</span>
              </h1>
              ...
              <div className="w-full flex justify-center lg:justify-start">
                <button onClick={() => scrollTo(depotRef)} className="relative group overflow-hidden bg-gradient-to-b from-[#d4af37] via-[#c58158] to-[#8c5a3d] text-[#1a0f0a] px-8 py-4 lg:py-6 font-black uppercase tracking-[0.15em] rounded-lg shadow-[0_8px_0_#3d291f] active:translate-y-[2px] transition-all flex items-center gap-4 italic text-base sm:text-lg">
                  <span className="relative z-10">TURN THE PRESSURE UP</span><ArrowRight size={22} /><div className="btn-glare-overlay" />
                </button>
              </div>
            </ScrollReveal>
          </div>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-24 items-center">
              <div className="space-y-6 lg:space-y-12">
                <div className="bg-[#2a1b15]/30 border-l-4 border-[#d4af37] p-6 lg:p-10 shadow-xl">
                  <div className="flex items-center gap-6 mb-4 text-[#d4af37]"><BatteryCharging size={32} strokeWidth={2.5} /><h3 className="text-2xl lg:text-3xl font-black uppercase italic text-white">Sleep affects energy.</h3></div>
                  <div className="flex items-center gap-6 mb-4 text-[#c58158]"><Zap size={32} strokeWidth={2.5} /><h3 className="text-2xl lg:text-3xl font-black uppercase italic text-white">Energy affects confidence.</h3></div>
                  <div className="flex items-center gap-6 text-white"><Trophy size={32} strokeWidth={2.5} /><h3 className="text-2xl lg:text-3xl font-black uppercase italic text-[#d4af37] underline decoration-[#d4af37]/30 underline-offset-8">Confidence affects everything.</h3></div>
                </div>
                <div className="space-y-6">
                  <p className="text-[#f4e4bc] text-lg lg:text-xl font-bold italic leading-relaxed">This formula was designed to support the everyday concerns men quietly deal with — nighttime interruptions, stamina, recovery, and overall vitality. When your routine feels steady, your day feels easier.</p>
                  <p className="text-[#c58158] font-black uppercase tracking-[0.2em] text-xs italic">No hype. No miracle promises. Just a blend created to support how men want to feel.</p>
                </div>
              </div>
              <div className="bg-[#1a0f0a] border-2 border-[#c58158]/20 p-8 lg:p-12 relative group">
                <div className="absolute inset-0 opacity-[0.05] pointer-events-none" style={{ backgroundImage: `url("https://images.travelprox.com/mrplumberman/herowall.png")`, backgroundSize: 'cover' }} />
                ...
                <ul className="space-y-6 relative z-10">
                  {['restless sleep', 'inconsistent energy', 'performance confidence', 'aging-related changes', 'daily stamina'].map((item, idx) => (
                    <li key={idx} className="flex items-center gap-4 group/item">
                      <div className="w-6 h-6 border border-[#d4af37]/40 rounded-sm flex items-center justify-center bg-[#d4af37]/5"><CheckCircle2 size={14} className="text-[#d4af37]" /></div>
      ...
      <section className="px-6 py-24 bg-[#1a0f0a] border-t border-[#c58158]/10">
        <ScrollReveal>
          <div className="max-w-4xl mx-auto">
            <div className="bg-[#2a1b15]/40 border-2 border-[#d4af37]/20 p-8 lg:p-16 rounded-sm shadow-2xl relative">
               ...
               <div className="relative z-10 space-y-8 text-center lg:text-left">
                 ...
                 <h3 className="text-3xl lg:text-5xl font-black uppercase italic text-white">THE STORY BEHIND <br/><span className="text-[#c58158]">THE SPEC.</span></h3>
                 ...
                 <div className="pt-10 border-t border-[#c58158]/20"><p className="text-2xl font-black text-white italic tracking-widest">— Rahj, Founder</p></div>
               </div>
            </div>
          </div>
        </ScrollReveal>
        ...
        <div className="max-w-7xl mx-auto">
          <ScrollReveal>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              {[
                { id: 'p', name: "PRESSURE", sub: "Prostate Support", price: 39, img: "https://images.travelprox.com/mrplumberman/pressure.png", benefits: ["Fewer nighttime disruptions", "Better mornings", "Wake up feeling rested"], priceId: 'price_1SwvOSKFN6WMOhlF5xerUfID', subPriceId: 'price_1SwvhCKFN6WMOhlFqFQiSYH6' },
                { id: 't', name: "PRIME TIME", sub: "T-Formula", price: 59, img: "https://images.travelprox.com/mrplumberman/primeheat.png", benefits: ["Support stamina", "Performance confidence", "Steady daily energy"], priceId: 'price_1SwvR8KFN6WMOhlFXD9hxqXi', subPriceId: 'price_1SwvkOKFN6WMOhlFRkxiPaPq' },
                { id: 'c', name: "THE OVERHAUL", sub: "Combo Pack", price: 97, img: "https://images.travelprox.com/mrplumberman/symbol.png", benefits: ["Complete energy overhaul", "Maximum performance", "Full routine maintenance"], tag: "Best Value", priceId: 'price_1SwvX4KFN6WMOhlFXU8Bs0lt', subPriceId: 'price_1SwvmEKFN6WMOhlFzNkPcl3U' }
              ].map(p => (
                <div key={p.id} className="bg-[#2a1b15]/40 border-2 border-[#c58158]/20 p-8 hover:border-[#d4af37] transition duration-500 flex flex-col items-center group rounded-sm shadow-2xl relative">
                  ...
                  <h3 className="text-3xl font-black text-white uppercase mb-2 italic">{p.name}</h3>
                  ...
                  <ul className="w-full space-y-3 mb-10 border-t border-[#c58158]/10 pt-6">
                    {p.benefits.map((b, i) => (<li key={i} className="flex items-start gap-3"><CheckCircle2 size={12} className="text-[#d4af37] mt-0.5 shrink-0" /><span className="text-[11px] text-[#f4e4bc] font-bold uppercase tracking-widest leading-snug">{b}</span></li>))}
                  </ul>
                  <div className="mt-auto w-full space-y-6 pt-8 border-t border-[#c58158]/20">
                    <div className="text-center"><p className="text-4xl font-black italic text-white mb-1">${p.price}</p><span className="text-[10px] text-[#c58158] font-black uppercase tracking-widest">Free Express Shipping</span></div>
                    <button onClick={() => handlePurchase(p.priceId, p.id, "payment")} disabled={isPurchasing === p.id} className="relative w-full bg-gradient-to-b from-[#d4af37] to-[#c58158] text-[#1a0f0a] py-4 font-black uppercase text-xs hover:brightness-110 shadow-[0_8px_0_#3d291f] active:translate-y-[8px] transition-all flex items-center justify-center gap-3">
                      <span className="relative z-10 flex items-center gap-3">{isPurchasing === p.id ? <Loader2 size={16} className="animate-spin" /> : <Package size={16} />}{isPurchasing === p.id ? 'Processing...' : 'Add To Kit'}</span><div className="btn-glare-overlay" />
                    </button>
                    <button onClick={() => handlePurchase(p.subPriceId, p.id + '_sub', "subscription")} disabled={isPurchasing === p.id + '_sub'} className="relative w-full border border-[#c58158]/40 bg-transparent text-[#d4af37] py-2 font-black uppercase text-[9px] hover:bg-[#c58158]/10 italic flex items-center justify-center gap-2 overflow-hidden text-center"><span className="relative z-10">{isPurchasing === p.id + '_sub' ? <Loader2 size={12} className="animate-spin" /> : <RefreshCcw size={12} />} Subscribe & Save 10%</span></button>
                  </div>
                </div>
              ))}
            </div>
      ...
      <footer className="bg-[#0f0a08] py-16 px-6 lg:px-10 border-t border-[#c58158]/10 text-center flex flex-col items-center">
        ...
        <div className="flex flex-col md:flex-row items-center gap-6 mb-10 text-[10px] font-black uppercase tracking-widest italic text-[#c58158]/60">
          ...
          <div className="flex items-center gap-2"><Mail size={14} className="text-[#d4af37]" /><span className="text-white/90 font-sans">hello@mrplumberman.com</span></div>
        </div>
        <div className="max-w-3xl mb-10 p-6 border border-[#c58158]/20 bg-[#1a0f0a]/50">
          <p className="text-[10px] font-bold text-[#c58158]/80 leading-relaxed italic font-sans">These statements have not been evaluated by the Food and Drug Administration. This product is not intended to diagnose, treat, cure, or prevent any disease. Consult a physician before use, especially if you have a medical condition or are taking medication.</p>
        </div>
      ...
      {typeof window !== "undefined" && window.location.hostname.includes("vercel.app") && (
        <button onClick={() => handlePurchase("price_1T09ZwKFN6WMOhlFYnV14aXP", "dev_test", "payment")} className="fixed bottom-24 right-6 z-[160] bg-black text-white px-4 py-3 text-xs font-black uppercase tracking-widest border-2 border-white shadow-2xl">DEV TEST CHECKOUT</button>
      )}
    </>
  );
};
  ...
  return (
    <div className="min-h-screen bg-[#1a0f0a] flex items-center justify-center px-6 text-center">
      ...
      <div className="max-w-2xl w-full relative z-10 space-y-10">
        ...
        <button onClick={() => navigate('home')} className="relative group overflow-hidden bg-gradient-to-b from-[#d4af37] to-[#c58158] text-[#1a0f0a] px-12 py-5 font-black uppercase rounded-lg shadow-[0_8px_0_#3d291f] flex items-center gap-3 italic mx-auto"><HomeIcon size={20} /> RETURN TO DEPOT<div className="btn-glare-overlay" /></button>
      </div>
    </div>
  );
};
